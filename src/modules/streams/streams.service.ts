...
export class StreamsService {
  ...
  async start(
    ...
    return this.generateToken(stream.id, RtcRole.PUBLISHER);
  }
  ...
  async view(reqUser: User, id: Stream['id']): Promise<StreamResponse> {
    ...
    return this.generateToken(stream.id, RtcRole.SUBSCRIBER);
  }
  ...
  private generateToken(
    ...
    privilegeExpireInSeconds = 3600,
  ): StreamResponse {
    const { appID, appCertificate } = this.configService.getOrThrow('agora', {
      infer: true,
    });
